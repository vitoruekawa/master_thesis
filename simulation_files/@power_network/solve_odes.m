function out = solve_odes(obj, t, u, idx_u, fault, x, xkg, xk, V0, I0, linear, options)
    ...
    for i = 1:numel(t_simulated) - 1
        ...
        % odeoptions = odeset('Mass', Mf, 'OutputFcn', r);
        ...
        y = sol.y(:, end);
        %%% Vitor's code
        Vpre = y(nx + (1:numel(idx_simulated_bus)));
        half1 = Vpre(1:end/2); half2 = Vpre(end/2 + 1: end);
        V = zeros(numel(Vpre), 1); 
        V(1:2:end) = half1; V(2:2:end) = half2;
        %%%
        x0 = y(1:nx);
        V0 = Ymat_reproduce * V;
        I0 = Ymat_all * V0;
